require 'xmlsimple'
class Mobile::PackagesController < Mobile::SiteController
  ...
  def index
    ...
    response = call_api("#{Mobile::SiteController::API_URL}/packages.xml", {:content_type=>"text/xml", :body=>"<?xml version='1.0' encoding='UTF-8'?><user><auth_token>"+session[:auth_token].to_s+"</auth_token></user>"})
    xml = response.body
    @tem = xml
    #xml.gsub(/(>)\s*(<)/, '\1\2').gsub(/\n+/, "")
  ...
  def show
    if !params[:id].nil? && !params[:auth_token].nil?
      ...
      clear_loggedin_user
      response = call_api("#{Mobile::SiteController::API_URL}/auth/#{params[:auth_token]}.xml")
      xml = response.body
      @temp = xml
      response_tag = XmlSimple.xml_in(xml)
      if response_tag != nil && response_tag["code"].to_s == "200"
        ...
        response = call_api("#{Mobile::SiteController::API_URL}/packages/#{params[:id]}.xml", {:content_type => 'text/xml', :body=>"<?xml version='1.0' encoding='UTF-8'?><user><auth_token>"+response_tag["user"][0]["auth_token"].to_s+"</auth_token></user>"})
        xml = response.body
        @tem = xml
  ...
  def redeem
    ...
    @message = "No details found";
    @error = "no"
    response = call_api("#{Mobile::SiteController::API_URL}/packages/redeem/#{params[:id]}.xml", {:content_type => 'text/xml', :body=>"<?xml version='1.0' encoding='UTF-8'?><user><auth_token>"+session[:auth_token].to_s+"</auth_token></user>"})
    xml = response.body    
  ...
  def verify_code
    ...
    @error = ""
    @merchant_message = ""
    response = call_api("#{Mobile::SiteController::API_URL}/packages/verify/#{params[:id]}.xml", {:content_type => 'text/xml', 
